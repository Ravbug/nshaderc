...
{
	vec3 lightPos = (u_view * vec4(u_lightPosRadius[_idx].xyz, 1.0)).xyz;
...
void main()
{
	...
	vec3 color = toLinear(texture(s_texColor, v_texcoord0)).xyz;

	vec3 ambient = toGamma(ambientColor * color);
	vec3 diffuse = toGamma(lightColor * color);
	outcolor.xyz = clamp(ambient + diffuse, 0.0, 1.0);

	outcolor.w = u_color.w;
}
