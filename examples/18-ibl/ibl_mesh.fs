...
void main()
{
	...
	vec3 cubeR = normalize((mtx * vec4(vr, 0.0)).xyz);
	vec3 cubeN = normalize((mtx * vec4(nn, 0.0)).xyz);
	cubeR = fixCubeLookup(cubeR, mip, 256.0);

	vec3 radiance    = toLinear(textureLod(s_texCube, cubeR, mip).xyz);
	vec3 irradiance  = toLinear(texture(s_texCubeIrr, cubeN).xyz);
