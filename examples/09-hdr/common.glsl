...
#define BLUR9_WEIGHT(_x) (_BLUR9_WEIGHT_##_x/_BLUR9_NORMALIZE)
	...
	vec4 blur;
	blur  = texture(_sampler, _uv0)*BLUR9_WEIGHT(0);
	blur += texture(_sampler, _uv1.xy)*BLUR9_WEIGHT(1);
	blur += texture(_sampler, _uv1.zw)*BLUR9_WEIGHT(1);
	blur += texture(_sampler, _uv2.xy)*BLUR9_WEIGHT(2);
	blur += texture(_sampler, _uv2.zw)*BLUR9_WEIGHT(2);
	blur += texture(_sampler, _uv3.xy)*BLUR9_WEIGHT(3);
	blur += texture(_sampler, _uv3.zw)*BLUR9_WEIGHT(3);
	blur += texture(_sampler, _uv4.xy)*BLUR9_WEIGHT(4);
	blur += texture(_sampler, _uv4.zw)*BLUR9_WEIGHT(4);
	return blur;
}
