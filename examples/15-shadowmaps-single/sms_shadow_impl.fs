...
layout(location=1) in vec4 v_shadowcoord;
...
#if SHADOW_PACKED_DEPTH
	return step(texCoord.z-_bias, unpackRgbaToFloat(texture(_sampler, texCoord.xy) ) );
#else
	return texture(_sampler, vec3(texCoord.xy, texCoord.z-_bias) );
#endif // SHADOW_PACKED_DEPTH
}
